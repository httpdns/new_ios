...
- (NSString *)getHostByName:(NSString *)url
{
    @synchronized(self)
    {
        ...
        NSArray* ipsVector = [[MSDKDns sharedInstance] WGGetHostByName:hostName];
        if (ipsVector && ipsVector.count > 1)
        {
            if (![ipsVector[1] isEqualToString:@"0"]) {
                ipAdress = [NSString stringWithFormat:@"[%@]",ipsVector[1]];
            } else {
                ipAdress = ipsVector[0];
            }
